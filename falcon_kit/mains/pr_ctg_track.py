...
def try_run_track_reads(n_core, base_dir, min_len, bestn, assemblyDir, correctedReadDir, correctReadDbName):
    io.LOG('starting track_reads')
    pread_dir = os.path.abspath( os.path.join(base_dir, correctedReadDir) )
    file_list = glob.glob( os.path.join( pread_dir, "*.*.las") )
    io.LOG('file list: %r' % file_list)
    db_fn = os.path.join( pread_dir, correctReadDbName )
...
def track_reads(n_core, base_dir, min_len, bestn, debug, silent, stream, assemblyDir, correctedReadDir, correctReadDbName):
    if debug:
        n_core = 0
        silent = False
    if silent:
    ...
    try_run_track_reads(n_core, base_dir, min_len, bestn, assemblyDir, correctedReadDir, correctReadDbName)
