...
def run_tr_stage1(db_fn, fn, min_len, bestn, rid_to_ctg):
    cmd = "/nfs/users/nfs_m/mm21/programs/FALCON-integrate/DALIGNER/LA4Falcon -m %s %s" % (db_fn, fn)
    reader = Reader(cmd)
    with reader:
...
def run_track_reads(exe_pool, base_dir, file_list, min_len, bestn, db_fn, assemblyDir):
    io.LOG('preparing tr_stage1')
    io.logstats()
    asm_dir = os.path.abspath( os.path.join(base_dir, assemblyDir) )
...
def try_run_track_reads(n_core, base_dir, min_len, bestn, assemblyDir, rawReadDir, rawReadDbName):
    io.LOG('starting track_reads')

    rawread_dir = os.path.abspath( os.path.join(base_dir, rawReadDir) )

    # better logic for finding the las files path or move the logic to extern (taking the --fofn option?)
    file_list = glob.glob( os.path.join( rawread_dir, "*.*.las") )
    io.LOG('file list: %r' % file_list)

    # same, shoud we decide this as a parameter
    db_fn = os.path.join( rawread_dir, rawReadDbName )
    n_core = min(n_core, len(file_list))
    exe_pool = Pool(n_core)
    try:
        run_track_reads(exe_pool, base_dir, file_list, min_len, bestn, db_fn, assemblyDir)
...
def track_reads(n_core, base_dir, min_len, bestn, debug, silent, stream, assemblyDir, rawReadDir, rawReadDbName):
    if debug:
        n_core = 0
        silent = False
    if silent:
    ...
    try_run_track_reads(n_core, base_dir, min_len, bestn, assemblyDir, rawReadDir, rawReadDbName)
